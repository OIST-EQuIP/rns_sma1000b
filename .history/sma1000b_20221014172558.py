...
class SMA1000B:
    ...
    def set_power_sweep_list(self, pow_list, dwell_list=0.001):
        ...
        pow_list = 10 * np.log10(np.power(pow_list, 2) * 1000 / 50)
        assert -145 <= min(pow_list)
        assert -max(pow_list) <= 36
        pow_str = ", ".join([str(pow) + "dBm" for pow in pow_list])
        self.instrument.write(f"SOUR1:LIST:POW {pow_str}")

        # dwell_list needs to be converted into s
        dwell_list = np.array
        if not isinstance(dwell_list, (tuple, list, np.ndarray)):
            assert 0.001 <= dwell_list and dwell_list <= 100
            dwell_str = str(dwell_list)
        else:
            assert 0.001 <= min(dwell_list)
            assert max(dwell_list) <= 100
            dwell_str = ", ".join([str(dwell) + "s" for dwell in dwell_list])
