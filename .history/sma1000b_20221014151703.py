...
class SMA1000B:
    ...
    def set_power_sweep_list(self, pow_list, dwell_list):
        pow_list = ", ".join([str(pow) + " V" for pow in pow_list])
        self.instrument.write(f"SOUR1:LIST:POW {pow_list}")
        dwell_list = ", ".join(dwell_list)
        self.instrument.write(f"SOUR1:LIST:DWELL:LIST {pow_list}")
